...
def main() -> None:
    ...
    n = motion_lib._num_unique_motions
    if args.max_motions:
        n = min(n, args.max_motions)
    print(f"motion_lib sees {motion_lib._num_unique_motions} motions, verifying {n}")
    ...
    for i in range(n):
        ...
        length = motion_lib._motion_lengths[i]
        times = torch.arange(0, float(length), 1.0 / 30.0, device=args.device)
        ids = torch.full((times.shape[0],), i, dtype=torch.long, device=args.device)
